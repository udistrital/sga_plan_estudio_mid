...
import (
	...
	"github.com/udistrital/sga_plan_estudio_mid/process/plan_estudio_visualizacion_documento"
...
// SERVICIOS
func PostBaseStudyPlan(data []byte) requestresponse.APIResponse {
	var studyPlanRequest map[string]interface{}
	const editionApprovalStatus = 1

	if err := json.Unmarshal(data, &studyPlanRequest); err == nil {
		if status, errStatus := getApprovalStatus(editionApprovalStatus); errStatus == nil {
			...
			if newPlan, errPlan := createStudyPlan(studyPlanRequest); errPlan == nil {
				return requestresponse.APIResponseDTO(true, 201, newPlan, "Created")
			} else {
				return requestresponse.APIResponseDTO(false, 400, nil, "Error al crear el plan de estudios")
			}
		} else {
			return requestresponse.APIResponseDTO(false, 404, nil, "Estado aprobación del plan de estudios no encontrado")
		}
	} else {
...
func GetStudyPlanVisualization(idPlan int64) requestresponse.APIResponse {
	...
	if errPlan == nil && resStudyPlan["Success"] == true {
		...
		if errorClass != nil {
			...
			return requestresponse.APIResponseDTO(false, 404, nil, errorClass.Error())
		}
		...
		if planData["EsPlanEstudioPadre"] == true {
			...
			if errPlanVisualization == nil {
				return requestresponse.APIResponseDTO(true, 200, visualizationData, "Query successful")
			} else {
				logs.Error(errPlanVisualization.Error())
				return requestresponse.APIResponseDTO(false, 404, nil, errPlanVisualization.Error())
			}
		} else {
			visualizationData, errPlanVisualization := getChildStudyPlanVisualization(planData, classificationsData)
			if errPlanVisualization == nil {
				return requestresponse.APIResponseDTO(true, 200, visualizationData)
			} else {
				logs.Error(errPlanVisualization.Error())
				return requestresponse.APIResponseDTO(false, 404, nil, errPlanVisualization.Error())
			}
		...
		requestresponse.APIResponseDTO(false, 404, nil, errPlan.Error())
	}
	return requestresponse.APIResponseDTO(false, 404, nil, "Error service GetStudyPlanVisualization: The request contains an incorrect parameter or no record exist")
}

func PostGenerarDocumentoPlanEstudio(dataBody []byte) requestresponse.APIResponse {
	var data map[string]interface{}

	if parseErr := json.Unmarshal(dataBody, &data); parseErr == nil {
		...
		if pdf.Err() {
			...
			return requestresponse.APIResponseDTO(false, 400, nil, "Error al generar el documento del plan de estudios")
		}

		if pdf.Ok() {
			encodedFile := utils.EncodePDF(pdf)
			return requestresponse.APIResponseDTO(true, 200, encodedFile)
		}
	} else {
		return requestresponse.APIResponseDTO(false, 400, nil, parseErr.Error())
	}
	return requestresponse.APIResponseDTO(false, 400, nil, "Error service PostGenerarDocumentoPlanEstudio: The request contains an incorrect parameter")
}

func GetPlanPorDependenciaVinculacionTercero(terceroId int64) requestresponse.APIResponse {
	...
	*/
	...
	estadoVinculacionErr := request.GetJson("http://"+beego.AppConfig.String("TercerosService")+
		fmt.Sprintf("vinculacion?query=Activo:true,DependenciaId__gt:0,CargoId__in:312|320,tercero_principal_id:%v", terceroId), &estadoVinculacion)
	if estadoVinculacionErr != nil || fmt.Sprintf("%v", estadoVinculacion) == "[map[]]" {
		...
		return requestresponse.APIResponseDTO(false, 404, nil, "Error service GetPlanPorDependenciaVinculacionTercero: Tercero relationship is empty")
	}
	/*
	...
	*/
	...
	if len(dependencias) == 0 {
		return requestresponse.APIResponseDTO(false, 404, nil, "Error service GetPlanPorDependenciaVinculacionTercero: Tercero without dependencies")
	}
	...
	return requestresponse.APIResponseDTO(true, 200, plans)
}
